...
/**
 * A parser which matches a sequence of tokens in a token list, based both on id and data. If the sequence of tokens cannot be matched, this parser will fail. This parser only accepts a tokens input.
 */
public class TokensWithDataParser extends MultiParser<Token<?>> {
    /**
     * The tokens list.
     */
     ...
     */
    public TokensWithDataParser(Token<?>... tokens) {
        this.tokens = tokens;

        setParserName("tokensWithData");
    }
    ...
    public ParserState<Token<?>[]> parse(ParserState<?> state) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            if (!tokens[i].equals(truncated[i])) {
